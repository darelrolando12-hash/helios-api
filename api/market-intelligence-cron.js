...
async function fetchLiveQuote(ticker) {
  ...
  // 1. Prev day close — DB-first (eliminates /v2/aggs/prev on every 60s cycle)
  let prevClose = await getDBPrevClose(ticker);
  let prevVol   = 0;

  if (!prevClose) {
    // DB miss — fall back to Polygon /prev (only when DB is cold/missing)
    ...
    prevClose = prevBar?.c ?? 0;
    prevVol   = prevBar?.v ?? 0;
    if (prevClose) console.log(`[market-intel] ${ticker} prev close from Polygon (DB miss)`);
  } else {
    console.log(`[market-intel] ${ticker} prev close from DB: ${prevClose}`);
  }
