...
export default async function handler(req, res) {
  ...
  if (isMarketHours() && !manualOverride) {
    ...
    return res.status(200).json({
      skipped: true,
      reason:  `Market hours (7AM–4PM CT) — cron blocked to protect rate limits. Current CT time: ${ctNow}. Use ?force=true to override.`,
    });
  }
  ...
  // ── Phase A: HV + ADV (all 24 tickers, serial, 350ms gap) ─────────────────
  ...
  // ── Phase B: Calibration priors (13 tickers, intraday fetches) ────────────
  ...
  // ── Phase C: Expiry dates (all 24 tickers, serial, 250ms gap) ─────────────
