module.exports = async function handler(req, res) {
  ...
  if (!polygonKey) {
    return res.status(500).json({ error: 'POLYGON_API_KEY not configured', expiryDates: [], contracts: [] });
  }
  ...
  if (expiration) {
    try {
      ...
      if (!callsRes.ok || !putsRes.ok) return res.status(200).json({ contracts: [], symbol: sym, spot: 0, expiration });
      const [callsData, putsData] = await Promise.all([callsRes.json(), putsRes.json()]);
      let spot = callsData.results?.[0]?.underlying_asset?.price || putsData.results?.[0]?.underlying_asset?.price || 0;
      const contractMap = new Map();
      (callsData.results || []).forEach(c => {
        const strike = c.details?.strike_price;
        if (!strike) return;
        if (!contractMap.has(strike)) contractMap.set(strike, { strike, callBid:0,callAsk:0,callIV:0,callOI:0,callVolume:0,putBid:0,putAsk:0,putIV:0,putOI:0,putVolume:0,atm:false,itmCall:false,itmPut:false });
        const ct = contractMap.get(strike);
        ct.callBid = c.market_data?.bid||0; ct.callAsk = c.market_data?.ask||0;
        ct.callIV = c.greeks?.implied_volatility||0; ct.callOI = c.open_interest||0; ct.callVolume = c.volume||0;
      });
      (putsData.results || []).forEach(p => {
        ...
        if (!contractMap.has(strike)) contractMap.set(strike, { strike, callBid:0,callAsk:0,callIV:0,callOI:0,callVolume:0,putBid:0,putAsk:0,putIV:0,putOI:0,putVolume:0,atm:false,itmCall:false,itmPut:false });
        const ct = contractMap.get(strike);
        ct.putBid = p.market_data?.bid||0; ct.putAsk = p.market_data?.ask||0;
        ct.putIV = p.greeks?.implied_volatility||0; ct.putOI = p.open_interest||0; ct.putVolume = p.volume||0;
      });
      const contracts = Array.from(contractMap.values()).sort((a,b)=>a.strike-b.strike).map(c => {
        const step = spot < 10 ? 0.5 : spot < 50 ? 1 : spot < 200 ? 5 : 10;
        return { ...c, atm: Math.abs(c.strike-spot) < step*0.6, itmCall: c.strike<spot, itmPut: c.strike>spot };
      });
  ...
  return res.status(400).json({ error: 'Either datesOnly=true or expiration parameter required' });
};
};
