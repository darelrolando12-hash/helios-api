...
async function readDBDailyData(symbol) {
  ...
  try {
    ...
    if (ageMs > 7 * 24 * 60 * 60 * 1000) {
      console.warn(`[ghost-resolve.js] DB daily_market_data stale for ${symbol} (${row.computed_date})`);
      return null;
    }
    return row;
  } catch {
...
// Returns { price, intradayHigh, intradayLow } — CR-2: all from single Yahoo call, zero extra requests.
async function getLivePrice(ticker) {
  ...
  try {
    ...
    if (yahooRes.ok) {
      ...
      if (meta) {
        ...
        if (price > 0) {
          // CR-2: capture intraday high/low alongside current price — zero extra calls
          const intradayHigh = meta.regularMarketDayHigh ?? price;
          const intradayLow  = meta.regularMarketDayLow  ?? price;
          console.log(`[ghost-resolve.js] getLivePrice ${ticker}: price=${price} intradayHigh=${intradayHigh} intradayLow=${intradayLow}`);
          return { price, intradayHigh, intradayLow };
        }
      }
    }
  } catch (yErr) {
  ...
  if (prevClose > 0) {
    ...
    return { price: prevClose, intradayHigh: null, intradayLow: null };
  }

  console.warn(`[ghost-resolve.js] getLivePrice ${ticker}: all sources failed`);
  return { price: 0, intradayHigh: null, intradayLow: null };
}
...
export default async function handler(req, res) {
  ...
  let resolved = 0, skipped = 0, deadLettered = 0;
  const log = [];
  ...
  try {
    ...
    for (const call of pending) {
      try {
        // Guard 1: Dead letter — only genuine resolution attempts count
        ...
        // Guard 3: Minimum hold time (15 min) — skip WITHOUT incrementing
        ...
        // Guard 4: Market hours only — skip WITHOUT incrementing
        // Overnight cron runs must not burn attempt slots.
        if (!isMarketHours()) {
          skipped++;
          continue;
        }

        // Past both guards — this is a genuine resolution attempt. Increment now.
        await sbUpdate('ghost_calls', { id: call.id }, { resolve_attempts: attempts + 1 });

        // Fetch live price (CR-2: returns { price, intradayHigh, intradayLow })
        const priceResult  = await getLivePrice(call.ticker);
        ...
        // CR-2: max_move_pct — best intraday move for Brain Elite/Target/Base tier learning
        let maxMovePct = null;
        const { intradayHigh, intradayLow } = priceResult;
        if (entryPrice > 0) {
          if (call.direction === 'BULLISH' && intradayHigh !== null) {
            maxMovePct = parseFloat(((intradayHigh - entryPrice) / entryPrice * 100).toFixed(2));
          } else if (call.direction === 'BEARISH' && intradayLow !== null) {
            maxMovePct = parseFloat(((entryPrice - intradayLow) / entryPrice * 100).toFixed(2));
          }
        }

        const convictionScore     = (call.conviction ?? 5) / 10;
        ...
        if (maxMovePct !== null) resolvedUpdate.max_move_pct = maxMovePct;
        await sbUpdate('ghost_calls', { id: call.id }, resolvedUpdate);
